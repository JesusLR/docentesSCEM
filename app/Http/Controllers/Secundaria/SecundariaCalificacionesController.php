...
class SecundariaCalificacionesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $grupo = Secundaria_grupos::with('secundaria_materia','periodo',
            'secundaria_empleado','plan.programa.escuela.departamento.ubicacion')
            ->find($grupo_id);
    ...
    {
        $primaria_empleado_id = Auth::user()->secundaria_empleado->id;
        $perActual = Auth::user()->secundaria_empleado->escuela->departamento->perActual;
        ...
        $periodos = DB::table('secundaria_inscritos')
            ...
            ->where('secundaria_grupos.empleado_id_docente', '=', $primaria_empleado_id)
            ->get();
    ...
    public function getAlumnos(Request $request, $id)
    {
        if($request->ajax()){
            ...
            $alumnos = Secundaria_inscritos::select('secundaria_inscritos.id', 'secundaria_inscritos.curso_id', 'secundaria_inscritos.grupo_id',
            'secundaria_grupos.gpoGrado', 'secundaria_grupos.gpoClave', 'programas.progClave', 'periodos.perAnio', 'secundaria_materias.matNombre', 'planes.planClave',
    ...
    public function getGrupos(Request $request, $id)
    {
        ...
        if($request->ajax()){
            ...
            $primaria_empleado_id = Auth::user()->secundaria_empleado->id;
                ...
                ->select('secundaria_inscritos.grupo_id', DB::raw('count(*) as grupo_id, secundaria_inscritos.grupo_id'),
                'secundaria_grupos.gpoGrado', DB::raw('count(*) as gpoGrado, secundaria_grupos.gpoGrado'),
                ...
                'secundaria_materias.matClave', DB::raw('count(*) as matClave, secundaria_materias.matClave'))
                ->join('secundaria_grupos', 'secundaria_inscritos.grupo_id', '=', 'secundaria_grupos.id')
                ->join('cursos', 'secundaria_inscritos.curso_id', '=', 'cursos.id')
                ->join('cgt', 'cursos.cgt_id', '=', 'cgt.id')
                ->join('planes', 'cgt.plan_id', '=', 'planes.id')
    ...
    public function getMaterias2(Request $request, $id)
    {
        ...
        if($request->ajax()){
            ...
            if($usuarioLogueado == "GINAESTHER" || $usuarioLogueado == "DESARROLLO.SECUNDARIA"){
                ...
                ->select('secundaria_materias.matNombre', DB::raw('count(*) as matNombre, secundaria_materias.matNombre'),
                'secundaria_materias.id', DB::raw('count(*) as id, secundaria_materias.id'),
                'secundaria_inscritos.grupo_id', DB::raw('count(*) as grupo_id, secundaria_inscritos.grupo_id'))
                ->join('secundaria_grupos', 'secundaria_inscritos.grupo_id', '=', 'secundaria_grupos.id')
                ->join('cursos', 'secundaria_inscritos.curso_id', '=', 'cursos.id')
                ->join('cgt', 'cursos.cgt_id', '=', 'cgt.id')
                ->join('planes', 'cgt.plan_id', '=', 'planes.id')
    ...
    public function guardarCalificacion(Request $request)
    {
        ...
        public function edit_calificacion($id)
        {
            ...
            'secundaria_mes_evaluaciones.numero_evaluacion')
    ...
    public function update_calificacion(Request $request)
    {
        ...
        if (!empty($secundaria_inscrito_id)) {
            $contar = count($secundaria_inscrito_id);
            //dd($contar);
            for (
                $i = 0;
                $i < $contar;
