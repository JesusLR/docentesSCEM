...
<div class="row">
    <div class="col s12 ">
        {!! Form::open(['route' => 'guarda_calificacion.store', 'method' => 'POST', 'id' => 'form-guardar']) !!}
                                                    ...
                                                    <td>
                                                        @if ($puedeOrdinario)
                                                            @if (is_null($inscrito->calificacion->inscCalificacionOrdinario) || ($matPorcentajeOrdinario == 100 && $matPorcentajeParcial == 0) || $grupo->materia->matTipoAcreditacion == 'A')
                                                                ...
                                                                @else
                                                                @php
                                                                    $validacion = ($TERCER_PARCIAL) ? ($grupo->estado_act == 'C' || is_null($inscrito->calificacion->inscCalificacionParcial3)) : ($grupo->estado_act == 'C');
                                                                @endphp
                                                                    <?=($validacion) ? 'hidden' : '' ?>
                                                                @endif
        ...
        if (matTipoAcreditacion == 'N') {
            $('ul.tabs').on('click', 'a#tabOrdinarios', function(e) {
                ...
                if (calificaciones.length !== $("input[type=number][name^='calificaciones[inscFaltasParcial3]']").length) {
                    ...
                    $('ul.tabs').tabs('select_tab', 'tercerparcial');
                } else {
